...
Vi bruker [versjonshåndtereren `asdf`](https://asdf-vm.com).
Dette verktøyet gjør at datamaskinen din automatisk velger den rette versjonen av Node og Yarn å bruke i prosjektmappen og dens undermapper.
[Installer verktøyet i henhold til instruksjonene](https://asdf-vm.com/guide/getting-started.html).

Gå inn i den nye mappen `design-system`. Installer `asdf`-pluginene for Node og Yarn hvis du ikke allerede har gjort det.
...
Installer alle avhengigheter med `yarn`:
