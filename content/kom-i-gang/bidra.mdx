...
<BannerAlertBox variant="info" title="Feiler det på @entur/fonts?">
  <Paragraph>
    <CodeText>@entur/fonts</CodeText>-avhengigheten er litt spesiell, siden den
    inneholder våre lisensierte fonter, og krever et annet tilgangsnivå. Om du
    ikke har gjort det allerede, kreves det at du har lagt til den offentlige
    SSH-nøkkelen din i Bitbucket.
  </Paragraph>
  <Paragraph>
    Om du sitter på OSX kan du kopiere den til utklippstavla med
    terminal-kommandoen <CodeText>cat ~/.ssh/id_rsa.pub | pbcopy</CodeText>.
    Atlassian har også{' '}
    <Link href="https://confluence.atlassian.com/bitbucket/set-up-an-ssh-key-728138079.html">
      en fin guide
    </Link>{' '}
    for hvordan dette gjøres.
  </Paragraph>
  <Paragraph>
    Du kan legge til SSH-nøkkelen din på Bitbucket ved å gå til{' '}
    <Link href="https://bitbucket.org/account">innstillinger</Link>,så{' '}
    <CodeText>Security -> SSH keys</CodeText> og så trykke på "Add key".
    <br />
    Når SSH-nøkkelen er lagt til her kan du prøve å kjøre `yarn` på nytt igjen.
  </Paragraph>
</BannerAlertBox>
