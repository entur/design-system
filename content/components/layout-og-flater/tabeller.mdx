...
<Playground>
  {() => {
    const [firstname, setFirstname] = React.useState('Ola');
    const [lastname, setLastname] = React.useState('Nordmann');
    return (
      <Table fixed>
        <TableHead>
          <TableRow>
            <HeaderCell>Fornavn</HeaderCell>
            <HeaderCell>Etternavn</HeaderCell>
          </TableRow>
        </TableHead>
        <TableBody>
          <TableRow hover>
