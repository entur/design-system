...
<Playground style={{ width: '50%', margin: 'auto' }}>
  <GridContainer spacing="medium">
    <GridItem small={12}>
      <TextField label="Skriv her..." />
    </GridItem>
    <GridItem small={12}>
      <TextField label="Kan ikke redigeres" readOnly value="Read-only modus" />
    </GridItem>
    <GridItem small={12}>
      <TextField
        label="Ikon som prepend"
        prepend={<SearchIcon inline />}
        placeholder="Og placeholder støttes fortsatt"
      />
    </GridItem>
    <GridItem small={12} style={{ width: '15rem' }}>
      <TextField prepend="Fra" append="Kroner" label="Label" />
    </GridItem>
  </GridContainer>
</Playground>
...
<div>
  ...
  og validering. Se{' '}
  <LinkText as={Link} to="/komponenter/skjemaelementer/input-group">
    Input groups
  </LinkText>{' '}
