...
| Top-level `<Heading>` inside `ModalContent` | `title` prop on `ModalContent` |

`size` is **required** on `Modal` itself (`'extraSmall' | 'small' | 'medium' | 'large' | 'extraLarge'`), and separately on `ModalContent`. `title` is optional on both.
...
// After
<Modal open={showModal} onDismiss={close} size="small">
