...
OBS: Disse funksjonene kan gi typefeil når de brukes i `DatePicker`. Dette skjer f.eks når man brukes `NativeDateToTimeOrDateValue` siden navnte funksjon kan i teorien
returnere `Time` som er en type `DatePicker` ikke godtar. Brukes konverteringsfunksjonen korrekt vil dog dette problemet aldri faktisk oppstå.
...
<CodeBlock asExpandable expandableLabel="JS Date-eksempel">
  ...
  return (
    <DatePicker
      label="Choose date"
      selectedDate={nativeDateToTimeOrDateValue(date) as DateValue}
