...
Du kan starte en utviklingsversjon av dokumentasjonssiden ved å kjøre `yarn start:documentation` på rotnivå. Du kan starte en byggprosess for hver pakke også ved å gå inn i hver pakke (`cd packages/buttons` for eksempel), og kjøre `yarn start` der også.
...
All dokumentasjonen vår finner du i `content`-mappa til `documentation`under `apps`-workspace-et. Vi har lagt dem i mapper basert på hvilken del av siden den tilhører, og hver av filene er i såkalt `mdx`-format.
...
Du kan starte opp en dev-versjon av dokumentasjonssiden med `yarn start:documentation` på rotnivå, og en lokal file watcher som rekompilerer filene dine med `yarn start` i mappen der pakken du jobber på ligger.
...
Til slutt committer du endringen din med `git commit …` eller `yarn gc:format`. Vi bruker et verktøy som heter `commitizen` for å standardisere commit-meldinger.
`commitizen` aktiveres automatisk hvis du er i `master`-branch-en og kan ellers aktiveres med `yarn gc:format` («gc» står for git commit).
...
Ved push av endrigene dine vil en commit-linter sjekke om du har fulgt `conventional-commits`-stilen. Ved bruk av `yarn gc:format` sikrer du at
...
Opprett filen `TestBench.tsx` under `apps/code-playground/src`. Innholdet i denne filen vil vises i et minimalt miljø når man kjører `yarn start:code-playground`. Denne filen trackes ikke av git, og vil og skal dermed ikke oppdateres i git-historikken.
...
Vi har en [Jira-tavle](https://enturas.atlassian.net/jira/software/c/projects/ETU/boards/173) for å opprette oppgaver.
