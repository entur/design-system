...
Alle chips finnes i `small`, `medium` (default) og `large`. Størrelsene styres av `size`-prop-en.

```jsx
<div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
  <ActionChip size="small">Small</ActionChip>
  <ActionChip size="medium">Medium</ActionChip>
  <ActionChip size="large">Large</ActionChip>
</div>
```

Høyder, padding, ikonstørrelser og typografi kommer fra design tokens som følger `data-view-mode`.
Setter du `data-view-mode="compact"` eller `data-view-mode="display"` på et element lenger opp i DOM-en,
skalerer chips-ene innenfor hver `size`.
