...
export const TextFieldExample: React.FC = () => {
  ...
  const validateEmail = (value: string) => {
    if (!value)
      return { variant: 'error' as const, message: 'Email is required' };
    if (!/\S+@\S+\.\S+/.test(value))
      return { variant: 'error' as const, message: 'Invalid email format' };
    return { variant: 'success' as const, message: 'Email looks good!' };
  };

  const validatePassword = (value: string) => {
    if (!value)
      return { variant: 'error' as const, message: 'Password is required' };
    if (value.length < 8)
      return {
