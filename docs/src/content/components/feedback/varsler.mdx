---
name: Varsler
route: /komponenter/feedback/varsler
parent: Komponenter
menu: Feedback
---

import { Playground, Props } from 'docz';
import DoAndDont from '../../../components/DoAndDont';
import { BannerAlertBox, ToastAlertBox, SmallAlertBox } from '@entur/alert/src';
// import '@entur/alert/dist/styles.css';

# Varsler

Varsler brukes for å fange brukerens oppmerksomhet og inneholder korte viktige meldinger som de bør ta hensyn til.

<Playground>
  <BannerAlertBox
    variant="success"
    title="Systemene våre er oppe igjen!"
    style={{ marginBottom: '1em' }}
  >
    Etter planlagt nedetid, er systemene våre igjen 100 % operasjonelle!
  </BannerAlertBox>
  <BannerAlertBox
    variant="info"
    title="Vi bruker informasjonskapsler"
    style={{ marginBottom: '1em' }}
  >
    Ved å bruke denne siden godtar du at vi bruker informasjonskapsler.
  </BannerAlertBox>
  <BannerAlertBox
    variant="warning"
    title="Systemene våre er ustabile"
    style={{ marginBottom: '1em' }}
  >
    Systemene våre kan oppleves litt ustabile akkurat nå. Vi jobber med å fikse
    problemet!
  </BannerAlertBox>
  <BannerAlertBox
    variant="error"
    title="Kjøp kan ikke gjennomføres nå"
    style={{ marginBottom: '1em' }}
  >
    Vi har et teknisk problem akkurat nå. Du kan fortsatt bruke
    reiseplanleggeren, men kjøpsløsningen for billetter fungerer ikke.
  </BannerAlertBox>
</Playground>

## Props

<Props of={BannerAlertBox} />

## Toast-varsler

Toasts brukes for å bekrefte handlinger brukeren har gjort. De bør dukke opp av seg selv øverst i høyre hjørne, og skal forsvinne automatisk etter minst seks sekunder.

<Playground>
  <ToastAlertBox
    variant="success"
    title="Nå har du billett!"
    style={{ marginBottom: '1em' }}
  >
    Ha det gøy på togturen til Bergen!
  </ToastAlertBox>
  <ToastAlertBox
    variant="info"
    title="Toget går fra plattform 2"
    style={{ marginBottom: '1em' }}
  >
    Husk å sjekke plattform når du ankommer.
  </ToastAlertBox>
  <ToastAlertBox
    variant="warning"
    title="Buss for tog på deler av strekningen"
    style={{ marginBottom: '1em' }}
  >
    Det er buss for tog på strekningen Bergen-Voss. Følg skilting underveis.
  </ToastAlertBox>
  <ToastAlertBox
    variant="error"
    title="Kjøpet kunne ikke gjennomføres!"
    style={{ marginBottom: '1em' }}
  >
    Det ser ut til at det ikke var dekning på det kortet du prøvde å betale med.
  </ToastAlertBox>
</Playground>

## Små varsler

Små alert-bokser brukes når de skal plasseres inni en annen UI-komponent.

<Playground>
  <SmallAlertBox variant="success" style={{ marginBottom: '1em' }}>
    Kjøpet gikk i orden!
  </SmallAlertBox>
  <SmallAlertBox variant="info" style={{ marginBottom: '1em' }}>
    Toget går fra plattform 2
  </SmallAlertBox>
  <SmallAlertBox variant="warning" style={{ marginBottom: '1em' }}>
    Snøvær på Bergensbanen
  </SmallAlertBox>
  <SmallAlertBox variant="error" style={{ marginBottom: '1em' }}>
    Kjøpet kunne ikke gjennomføres!
  </SmallAlertBox>
</Playground>

<Playground>
  <SmallAlertBox variant="success" style={{ marginBottom: '1em' }}>
    Kjøpet gikk i orden!
  </SmallAlertBox>
  <SmallAlertBox variant="info" style={{ marginBottom: '1em' }}>
    Toget går fra plattform 2
  </SmallAlertBox>
  <SmallAlertBox variant="warning" style={{ marginBottom: '1em' }}>
    Snøvær på Bergensbanen
  </SmallAlertBox>
  <SmallAlertBox variant="error" style={{ marginBottom: '1em' }}>
    Kjøpet kunne ikke gjennomføres!
  </SmallAlertBox>
</Playground>

## Retningslinjer

### Varianter

#### Information

Informasjonsvarslingen er blå. Den gir brukeren relevante tips eller informasjon som man behøver å vite når man skal utføre en oppgave.

#### Success

Suksessvarselet er grønn og vises når en handling er vellykket. (F.eks. I en kjøpsflyt der betalingen ble gjennomført)

#### Warning

Advarselen er gul og brukes for å advare brukeren om viktig informasjon som trenger oppmerksomhet. (F.eks. Systemet er nede pga. vedlikehold eller avvik i reiseforslag).

#### Error

Feilvarslingen er rød. Den indikerer at noe gikk galt etter å ha utført en handling, eller for å varsle brukeren om en feil som trenger øyeblikkelig oppmerksomhet. (F.eks. Skjemaet kunne ikke lagres fordi det mangler data på noen felter)

## Prinsipper

<DoAndDont>
  <DoAndDont.Do>
    <ul>
      <li>Bruk varsler sparsomt</li>
      <li>Hold det kort og konsist</li>
      <li>Tillat brukeren å avvise et varsel der det er aktuelt</li>
      <li>Bruk standardikonet for info, suksess, advarsel og feil</li>
    </ul>
  </DoAndDont.Do>
  <DoAndDont.Dont>
    <ul>
      <li>Legg varsler som legges over eller flyter over innhold</li>
      <li>
        Ikke overdriv bruken av varsler. For mange vil enten overvelde eller
        irritere brukeren og vil sannsynligvis bli ignorert
      </li>
    </ul>
  </DoAndDont.Dont>
</DoAndDont>
